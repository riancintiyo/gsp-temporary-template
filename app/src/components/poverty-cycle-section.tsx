...
const items: AccordionItemData[] = [
    {
        ...
        content: (
            <>
                Occupations that related to <strong>STEM and STEAM</strong> will growth until <strong>10,4%</strong> compared to <strong>3,6%</strong> for non STEM occupations between 2024 and 2033. Moreover, more than 80% of jobs over the next decade will require STEM skills. Lastly, an additional{" "}
                <strong>653,000</strong> tech workers will be needed by 2030 to meet demand.
            </>
        ),
        tags: [
    ...
    },
    {
        ...
        content: (
            <>
                Shifting just <strong>1%</strong> workforce into STEM roles would add <strong>$57.4 billion</strong> to GDP (net present value over 20 years). Additionally, between 1960 and 2000, 75% of the growth in gross domestic product around the world was linked to increased math and science
                skills.
            </>
        ),
        tags: [
    ...
    },
    {
        ...
        content: (
            <>
                Early STEM intervention helps children broaden their horizon and aspirations, where at the age of <strong>7 years</strong> those factors are affected by stereotypes. Subsequently, teachers childhood experiences are grounded in the construction of identity. Through conversations and
                activities, those things can influence a child’s future aspirations and career.
            </>
        ),
        tags: [
            { label: "Science", color: "green" },
...
export function PovertyCycleSection() {
    return (
        <section className="relative w-full py-16 lg:pt-24 lg:pb-32 bg-white overflow-hidden font-sans">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center md:items-start">
                    ...
                    <div className="order-2 mt-2">
                        <Accordion type="multiple" className="w-full" defaultValue={["steam-occupation"]}>
                            {items.map((item) => (
                                <motion.div key={item.value} whileHover={{ scale: 1.015, y: -2 }} transition={{ type: "spring", stiffness: 380, damping: 26 }} className="mb-4 last:mb-0">
                                    <AccordionItem value={item.value} className="rounded-2xl border border-grey-100 bg-white px-6 py-2 shadow-webflow-dropshadow">
                    ...
                    <div className="order-1 flex flex-col items-center justify-center md:items-start md:justify-start  gap-8">
                        ...
                        <motion.div whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 380, damping: 26 }} className="relative w-full max-w-75 md:max-w-124 overflow-hidden rounded-2xl">
                            <Image
