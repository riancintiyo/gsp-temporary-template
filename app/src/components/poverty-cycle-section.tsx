...
const items: AccordionItemData[] = [
    {
        ...
        content: "Occupations that related to STEM and STEAM will growth until 10,4% compared to 3,6% for non STEM occupations between 2024 and 2033. Moreover, more than 80% of jobs over the next decade will require STEM skills. Lastly, an additional 653,000 tech workers will be needed by 2030 to meet demand.",
        tags: [
            ...
            <div className="relative z-10 mx-auto w-full max-w-360 px-8 lg:px-(--page-margin)">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center md:items-start">
                            ...
                            {items.map((item) => (
                                <motion.div key={item.value} whileHover={{ scale: 1.015, y: -2 }} transition={{ type: "spring", stiffness: 380, damping: 26 }} className="mb-4 last:mb-0">
                                    <AccordionItem value={item.value} className="rounded-2xl border border-grey-100 bg-white px-6 py-2 shadow-webflow-dropshadow">
                                        <AccordionTrigger className="text-lg font-bold text-grey-1200 hover:no-underline">
                                            <TextAnimate as="span" by="line" animation="fadeIn" startOnView className="text-lg font-bold text-grey-1200">
                                                {item.trigger}
                                            </TextAnimate>
                                        </AccordionTrigger>
                                        <AccordionContent>
                                            <TextAnimate as="p" by="line" animation="fadeIn" startOnView delay={0.04} className="text-sm text-grey-800 leading-relaxed mb-4">
                                                {item.content}
                                            </TextAnimate>
                    ...
                    <div className="order-1 flex flex-col items-center justify-center md:items-start md:justify-start  gap-8">
                        <TextAnimate as="h2" by="line" animation="fadeIn" startOnView className="text-4xl sm:text-5xl lg:text-5xl font-bold text-grey-1200 leading-tight text-center md:text-start">
                            {`Cycle of Poverty and Opportunities in Wonosobo`}
                        </TextAnimate>
